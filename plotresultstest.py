...
parser.add_argument('--dir', dest='out_dir', default='results' , help="Directory to store outputs")
...
def get_throughput(throughput, traffic, app, input_file):
	...
	if traffic not in throughput:
		throughput[traffic] = {}

	if 'realtime_bisection_bw' not in throughput[traffic]:
		throughput[traffic]['realtime_bisection_bw'] = {}
	if 'realtime_throughput' not in throughput[traffic]:
		throughput[traffic]['realtime_throughput'] = {}
	if 'accumulated_throughput' not in throughput[traffic]:
		throughput[traffic]['accumulated_throughput'] = {}
	if 'normalized_total_throughput' not in throughput[traffic]:
		throughput[traffic]['normalized_total_throughput'] = {}

	if app not in throughput[traffic]['realtime_bisection_bw']:
		throughput[traffic]['realtime_bisection_bw'][app] = {}
	if app not in throughput[traffic]['realtime_throughput']:
		throughput[traffic]['realtime_throughput'][app] = {}
	if app not in throughput[traffic]['accumulated_throughput']:
		throughput[traffic]['accumulated_throughput'][app] = {}
	if app not in throughput[traffic]['normalized_total_throughput']:
		throughput[traffic]['normalized_total_throughput'][app] = 0

	for i in range(args.duration + 1):
		if i not in throughput[traffic]['realtime_bisection_bw'][app]:
			throughput[traffic]['realtime_bisection_bw'][app][i] = 0
		if i not in throughput[traffic]['realtime_throughput'][app]:
			throughput[traffic]['realtime_throughput'][app][i] = 0
		if i not in throughput[traffic]['accumulated_throughput'][app]:
...
def get_average_bisection_bw(throughput, traffics, app):
	...
	for i in range(9):
		value_list.append(calculate_average(complete_list[(i * 3): (i * 3 + 3)]))
	return value_list
...
def get_utilization(utilization, traffic, app, input_file):
	...
	if traffic not in utilization:
		utilization[traffic] = {}
	if app not in utilization[traffic]:
	...
	for row in lines_list:
		...
		if iface_name.startswith('1'):
			if (int(row[0]) - first_second) <= args.duration:   # Take the good values only.
				if iface_name not in utilization[traffic][app]:
...
def get_link_utilization_ratio(utilization, traffics, app):
	...
	for i in range(9):
		value_list.append(calculate_average(complete_list[(i * 3): (i * 3 + 3)]))
	for i in range(9):
		average_list.append(calculate_average(num_list[(i * 3): (i * 3 + 3)]))
...
def plot_results():
	...
	index = np.arange(num_groups) + 0.18
	bar_width = 0.15
	plt.bar(index, ECMP_value_list, bar_width, color='w', edgecolor='black',label='ECMP')
	plt.bar(index + 1 * bar_width, Hedera_value_list, bar_width, color='w',edgecolor='black',hatch="-", label='Hedera')
	# plt.bar(index + 2 * bar_width, PureSDN_value_list, bar_width, color='g', label='PureSDN')
	plt.bar(index + 2 * bar_width, BFlows_value_list, bar_width, color='w',edgecolor='black',hatch="/", label='CLB')
	...
	for i in range(len(traffics_brief)):
		plt.subplot(num_raw, num_column, NO_subplot)
		y1 = get_value_list_3(utilization, traffics[(i * 3): (i * 3 + 3)], 'ECMP')
		y2 = get_value_list_3(utilization, traffics[(i * 3): (i * 3 + 3)], 'Hedera')
		# y3 = get_value_list_3(utilization, traffics[(i * 3): (i * 3 + 3)], 'PureSDN')
		y3 = get_value_list_3(utilization, traffics[(i * 3): (i * 3 + 3)], 'BFlows')
		# print "y1[10]:", y1[10]
		# print "y2[10]:", y2[10]
		# print "y3[10]:", y3[10]
		# print "y4[10]:", y4[10]
		...
		plt.plot(x, y3, 'r-', linewidth=2, label="CLB")
		plt.title('%s' % traffics_brief[i], fontsize='xx-large')
		plt.xlabel('Link Bandwidth Utilization Ratio', fontsize='x-large')
		plt.xlim(0, 1)
