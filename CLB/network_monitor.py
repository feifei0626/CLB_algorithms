...
class NetworkMonitor(app_manager.RyuApp):
	...
	def get_best_path_by_fnum(self, graph, paths):
		...
		for src in list(fnum_of_paths.keys()):
			for dst in list(fnum_of_paths[src].keys()):
				fnum_of_paths[src][dst] = {}

		# 计算每条路径的大流数目并选出大流数目最少的路径集.
		for src in paths:
	...
	def zero_dictionary(self, fnum_dict):
		for sw in list(fnum_dict.keys()):
			for port in list(fnum_dict[sw].keys()):
				fnum_dict[sw][port] = 0
	...
	def create_fnum_graph(self, fnum_dict):
		...
		try:
			...
			for link, port in list(link_to_port.items()):
				(src_dpid, dst_dpid) = link
				(src_port, dst_port) = port
				if src_dpid in fnum_dict and src_port in fnum_dict[src_dpid]:
