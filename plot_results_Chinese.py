...
def get_link_utilization_ratio(utilization, traffics, app):
	...
	for traffic in traffics:
		num = 0
		for interface in list(utilization[traffic][app].keys()):
			if utilization[traffic][app][interface]['LU_out'] == 1:
				num += 1
			if utilization[traffic][app][interface]['LU_in'] == 1:
				num += 1
		num_list.append(num)
		complete_list.append(float(num) / (len(list(utilization[traffic][app].keys())) * 2))
	for i in range(9):
		value_list.append(calculate_average(complete_list[(i * 20): (i * 20 + 20)]))
	for i in range(9):
...
def get_value_list_3(utilization, some_traffics, app):
	...
	for ratio in utilization_list:
		for seq in list(link_bandwidth_utilization.keys()):
			if ratio <= seq:
...
def plot_results():
	...
	plt.bar(index + 2 * bar_width, BFlows_value_list, bar_width, color='r', label='BFlows')
	...
	for i in range(len(traffics_brief)):
		...
		y3 = get_value_list_3(utilization, traffics[(i * 20): (i * 20 + 20)], 'BFlows')
		# print "y1[10]:", y1[10]
		# print "y2[10]:", y2[10]
		# print "y3[10]:", y3[10]
		# print "y4[10]:", y4[10]
		...
		plt.plot(x, y3, 'r-', linewidth=2, label="BFlows")
		plt.title('%s' % traffics_brief[i], fontsize='xx-large')
		plt.xlabel('链路带宽利用率', fontsize='xx-large', fontproperties=chinese_font)
		plt.xlim(0, 1)
